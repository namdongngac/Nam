...
void cregame()
{
	...
	createMul(Item_bullet,rand()%ConsoleW+1,0,2,300,countbullet,iFly);
	//xu li dan
	...
	if(score!=0&&score%40==0) sumBoss=true;
	if(sumBoss&&Boss.hp>0) sum(Boss,BulletBoss,countfly,tempBoss);
	if(Boss.hp==0)
	{
		score+=10;
		sumBoss=false;
